...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addMulti = function(rowid,pos,irow,checked){
		    var v = "<input role=\"checkbox\" type=\"checkbox\"" + " id=\"jqg_" + ts.p.id + "_" + rowid + "\" class=\"cbox u-chk\" name=\"jqg_" + ts.p.id + "_" + rowid + "\"" + (checked ? "checked=\"checked\"" : "") + "/>",
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
						...
						if($.fmatter) {
							...
							from=$.fmatter.util.NumberFormat(ts.p.page,fmt);
		...
		setPager = function (pgid, tp){
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class=' ui-icon-seek-first comn-btn' title='首页'></span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class=' ui-icon-seek-prev comn-btn' title='上一页'></span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class=' ui-icon-seek-next comn-btn' title='下一页'></span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class=' ui-icon-seek-end comn-btn' title='尾页'></span></td>";
		...
		if(this.p.multiselect) {
		    this.p.colNames.unshift("<input role='checkbox' id='cb_" + this.p.id + "' class='cbox u-chk' type='checkbox'/>");
		...
		if(rdata) {
			...
			this.each(function() {
				...
				while(k < datalen) {
					...
					if(gi) {
					    v = "<input role=\"checkbox\" type=\"checkbox\"" + " id=\"jqg_" + t.p.id + "_" + rowid + "\" class=\"cbox u-chk\"/>";
